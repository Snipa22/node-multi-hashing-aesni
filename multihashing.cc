...
NAN_METHOD(cryptonight) {
    ...
    init_ctx();
    switch (variant) {
       case 0: cryptonight_single_hash<MONERO_ITER, MONERO_MEMORY, MONERO_MASK, false, 0>(static_cast<const uint8_t*>(Buffer::Data(target)), Buffer::Length(target), output, ctx);
               break;
       case 1: cryptonight_single_hash<MONERO_ITER, MONERO_MEMORY, MONERO_MASK, false, 1>(static_cast<const uint8_t*>(Buffer::Data(target)), Buffer::Length(target), output, ctx);
               break;
...
class CNAsyncWorker : public Nan::AsyncWorker {
    ...
    public:
        ...
        void Execute () {
            ...
            switch (m_variant) {
                case 0: cryptonight_single_hash<MONERO_ITER, MONERO_MEMORY, MONERO_MASK, false, 0>(static_cast<const uint8_t*>(m_input), m_input_len, m_output, m_ctx);
                        break;
                case 1: cryptonight_single_hash<MONERO_ITER, MONERO_MEMORY, MONERO_MASK, false, 1>(static_cast<const uint8_t*>(m_input), m_input_len, m_output, m_ctx);
                        break;
                default: return THROW_ERROR_EXCEPTION("Unknown PoW variant");
            }
        }

...
let lr = lineReader.createInterface({
     input: fs.createReadStream('cryptonight-zls.txt')
});
...
lr.on('close', function(){
    if (testsFailed > 0){
        console.log(testsFailed + '/' + (testsPassed + testsFailed) + ' tests failed on: cryptonight-zls');
    } else {
        console.log(testsPassed + ' tests passed on: cryptonight-zls');
    }
});

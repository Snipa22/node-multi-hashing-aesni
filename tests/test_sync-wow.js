...
lr.on('line', function (line) {
     ...
     let result = multiHashing.cryptonight(Buffer.from(line_data[1], 'hex'), 12, parseInt(line_data[2])).toString('hex');
...
lr.on('close', function(){
    if (testsFailed > 0){
        ...
        console.log(testsPassed + ' tests passed on: cryptonight-wow');
    }
});
